...
export const PieceItemCardRenderer: React.FC<PieceItemCardRendererProps> = ({ item, isPreview, chapterIndex = 0 }) => {
  ...
  const aspectRatioClass = useMemo(() => {
    if (pieceItem?.presentationStyle === 'card') {
  ...
  const cardClassName = useMemo(() => {
    return cn(
      ...
      pieceItem?.presentationStyle === 'doc' ? 'max-w-3xl' : 'max-w-md',
      editorSettings.background
    );
  }, [pieceItem?.presentationStyle, editorSettings.background, aspectRatioClass]);
              ...
              <div className="h-full w-full overflow-y-auto @container/content-card">
                  {pieceItem?.contentState === 'processing' ? (
                      ...
                      <PageContentRenderer
                          ...
                          presentationStyle={pieceItem?.presentationStyle || 'card'}
