...
export default function PieceReader({
  ...
  const { pages, currentPageIndex, isCalculating, goToPage, pageCount } = usePagination({
      ...
      return (
        <div
          className={cn(
            ...
            finalPresentationStyle === 'doc' ? 'aspect-[210/297] max-w-3xl' : 'max-w-md',
  ...
  const cardClassName = cn(
    ...
    finalPresentationStyle === 'card' && getAspectRatioClass(finalAspectRatio),
    finalPresentationStyle === 'doc' && 'aspect-[210/297] max-w-3xl',
    finalPresentationStyle === 'card' && 'max-w-md',
    editorSettings.background
  );

  // Preview mode - simple single page render
  if (isPreview) {
    const singlePage = {
      pageIndex: 0,
      items: allSegments,
    ...
    <div id="reader-veil" className="w-full h-full fixed inset-0 z-40" onMouseUp={handleTextSelection}>
      ...
      <div id="reader-studio-container" className="w-full h-full flex flex-col items-center justify-center p-4">
        <div
          ...
          className="relative w-full flex items-center justify-center min-h-0 group/reader"
          style={{
            height: 'calc(100vh - 56px - 56px - 2rem)'
          }}
        >
          ...
          <motion.div
            ...
            style={{
              containerType: 'size' // Enable CQI for card/doc
            }}
          >
            {isCalculating ? (
              ...
              <ContentPageRenderer
                ...
                displayLang1={displayLang1}
                displayLang2={displayLang2}
                showPageNumber={finalPresentationStyle === 'doc'}
              />
            ) : (
              <div className="flex items-center justify-center h-full text-center text-muted-foreground p-8">
