...
export const ReaderToolbar: React.FC<ReaderToolbarProps> = ({
  ...
  return (
    <div className="bg-background/90 border shadow-lg p-1.5 md:p-2 animate-in fade-in-0 slide-in-from-top-2 duration-300 rounded-lg flex flex-col gap-2">
      {bookTitle && (
        <div className="text-center font-headline text-sm font-semibold truncate px-2 text-primary">{bookTitle}</div>
      )}
      <TooltipProvider>
            ...
            <DropdownMenuContent>
              ...
              <DropdownMenuRadioGroup value={displayLang1} onValueChange={onDisplayLang1Change}>
                {availableLanguages.map((lang) => (
                  <DropdownMenuRadioItem key={`l1-${lang}`} value={lang}>
              ...
              <Tooltip key={opt.value}>
                <TooltipTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className={cn(buttonClass, settings.textAlign === opt.value && 'bg-muted')}
                  ...
                  <Tooltip key={opt.value}>
                    <TooltipTrigger asChild>
                      <Button
                        variant="ghost"
                        size="icon"
                        className={cn(buttonClass, settings.verticalAlign === opt.value && 'bg-muted')}
                        onClick={() => updateSetting('verticalAlign', opt.value)}
                      >
                        <Icon name={opt.icon} className={iconClass} />
                      </Button>
                    </TooltipTrigger>
                    <TooltipContent>
                      <p>{opt.value.replace('justify-', '')}</p>
                    </TooltipContent>
                  </Tooltip>
                ))}
              </div>
